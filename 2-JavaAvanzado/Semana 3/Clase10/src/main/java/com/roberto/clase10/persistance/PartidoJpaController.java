...
public class PartidoJpaController implements Serializable {
    ...
    public PartidoJpaController() {
        emf = Persistence.createEntityManagerFactory("clase10PU");
    } 
    ...
    public void destroy(Long id) throws NonexistentEntityException {
        ...
        try {
            ...
            try {
                ...
                throw new NonexistentEntityException("El partido con id " + id + " no existe", enfe);
            }
            em.remove(partido);
            em.getTransaction().commit();
        } finally {
    ...
    public void sumarVoto(Long partidoId) throws NonexistentEntityException{
        EntityManager em = null;
    try {
        em = getEntityManager();
        em.getTransaction().begin();
        ...
        Partido partidoActual = em.find(Partido.class, partidoId);

        if (partidoActual != null) {
           
            partidoActual.setVotos(partidoActual.getVotos() + 1);
           
            em.merge(partidoActual);
            em.getTransaction().commit();
        } else {
            throw new NonexistentEntityException("El partido con id " + partidoId + " no existe");
        }    
    } finally {
        if (em != null) {
            em.close();
